        ...
        for dl in utils.get_train_data(vocabulary, batch_size=FLAGS.batch_size, num_steps=FLAGS.num_steps):
            ...
            for i in range(shape[0]):
                for j in range(shape[1]):
                    idx = dl[i, j]
                    inputs[i, j] = dictionary[vocabulary[idx] if vocabulary[idx] in dictionary else "UNK"]
                    label[i, j] = dictionary[vocabulary[idx + 1] if (idx + 1 < len(vocabulary) and vocabulary[idx + 1] in dictionary) else "UNK"]

            feed_dict = {}
            feed_dict[model.X] = inputs
            feed_dict[model.Y] = label
